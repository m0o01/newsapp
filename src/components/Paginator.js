...
class Paginator extends Component {
    ...
    switchPage(active, direction){
        if(!active)
            return
        this.changePage((direction === "next") ? this.props.activePage + 1 : this.props.activePage - 1)
    }
    ...
    pagination(){
        ...
        for(let i = 1; i <= this.props.pages; i++)
        {
            if(i === this.props.activePage)
                template.push(<li className="page-item active" key={i}><span className="page-link" onClick={this.changePage.bind(this, i)} href="#">{i}</span></li>)
            else
                template.push(<li className="page-item" key={i}><span className="page-link" onClick={this.changePage.bind(this, i)} href="#">{i}</span></li>)
        }
        return template
    }
    ...
    render(){
        ...
        const nextActive = (this.props.activePage === this.props.pages) ? false : true
        return(
